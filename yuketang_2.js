...
(() => {
  ...
  const Config = {
    ...
    autoStartDelay: 10_000, // 页面加载 10 秒后自动开始刷课
    storageKeys: {        // 使用者勿动
  ...
  function createPanel() {
    ...
    const invokeStart = (mode = 'manual') => {
      if (hasStarted) return;
      hasStarted = true;
      clearAutoStartTimer();
      log(mode === 'auto'
        ? `页面加载 ${Math.round(Config.autoStartDelay / 1000)} 秒后自动开始刷课`
        : '启动中...');
    ...
    return {
      ...
      scheduleAutoStart(delay = Config.autoStartDelay) {
        if (hasStarted) return;
        clearAutoStartTimer();
        log(`已排程 ${Math.round(delay / 1000)} 秒后自动执行【开始刷课】`);
        autoStartTimer = setTimeout(() => invokeStart('auto'), delay);
      },
...
`;
  ...
  async function boot() {
    ...
    try {
      ...
      } else if (Utils.isSupportedLearningPage()) {
        panel.scheduleAutoStart();
      }
    } catch (err) {
